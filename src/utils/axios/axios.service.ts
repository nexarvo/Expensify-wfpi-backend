...
export class AxiosService {
  ...
  async get<T = any>(
    ...
    try {
      ...
      this._logger.info(
        `Successfully executed get http request with Url: ${url}, In ${elapsedTime}ms`,
      );
  ...
  async post(url: string, data: any, config: any): Promise<AxiosResponse> {
    try {
      ...
      this._logger.info(
        `Successfully executed post http request with Url: ${url}, In ${elapsedTime}ms`,
      );
  ...
  async delete<T = any>(
    ...
    try {
      ...
      this._logger.info(
        `Successfully executed delete http request with Url: ${url}, In ${elapsedTime}ms`,
      );

      return response;
    } catch (ex) {
      this._logger.info(
        `Failed to execute delete http request with Url: ${url}, ErrorMessage: ${ex.message}`,
      );
      return ex?.response;
    }
  }
