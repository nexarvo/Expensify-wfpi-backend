...
import { ListApiQueryDto } from '../core/dto/api-query-dto';
  ...
  export class WaitingForProposalIssuesService {
    ...
    > {
      try {
        this._logger.info(
          `Executing get waiting for proposal issues`,
        );
        ...
        if (response.status === HttpStatus.OK) {
          ...
          for (const issue of filteredIssues) {
            ...
            if (!searchTextFound) {
              const issueResult = new WaitingForProposalIssuesDisplayModel(
                issue.id,
                issue.title,
                issue.state,
                issue.created_at,
        ...
        this._logger.info(
          `Successfully executed get waiting for proposal issues`,
        );
      } catch (ex) {
        this._logger.error(
          `Failed to execute get waiting for proposal issues with error: ${ex}`,
        );
